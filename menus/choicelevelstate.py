...
class ChoiceLevelState(state.State):
    def __init__(self):
        ...
        for i in range(len(self.list_level_file)):
            ...
            self.widget_manager.add_widget(previewbutton.PreviewButton(0, self.top_button_y + i*120, 300, 100, "TC", res.tiled_button_sprite, 
                                                                              8, self.list_preview[i], 1, self.list_level_file[i][:-4], act = self.make_play_action(i)))
