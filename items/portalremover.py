...
from playeraction import portalremoveraction
import globalresources as res
from items import collectible


class PortalRemover(collectible.Collectible):
    def __init__(self, x: float, y: float):
        super().__init__(x, y)
        self._setup_collectible(0.6, 0.4, res.portal_remover_sprite)

    def update(self, dt: float):
        cols: list[player.Player] = self.manager.get_collision(self, player.Player)
        if len(cols) > 0:
            cols[0].add_action(portalremoveraction.PortalRemoverAction())
